...
├── assets/
│   ├── review-template.svg   # OGP画像のSVGテンプレート
│   └── fonts/
...
**成功時（終了コード 0）**
```json
{"ok": true, "filename": "a3f2c1d4e5b6....png"}
```

> `filename` のみを返す。`public/img/review/` のプレフィックスはLaravel側で付与する。
...
```svg
<!-- assets/review-template.svg の骨格（実装時に調整） -->
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630">
  <!-- 背景 -->
  <rect width="1200" height="630" fill="#0f172a"/>

  <!-- ネタバレバッジ（has_spoiler=true時はSVG要素に、falseは空文字列に置換） -->
  {{SPOILER_BADGE}}
...
| 変数名 | 説明 | 例 |
|--------|------|----|
| `OUTPUT_DIR` | PNG 保存先ディレクトリの絶対パス | `{hgs_re3設置パス}/public/img/review` |
| `FONT_PATH` | 日本語フォントファイルの絶対パス | `/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc` |
| `SVG_TEMPLATE_PATH` | SVGテンプレートの絶対パス | `{hgs_re3設置パス}/resources/ogp/review-template.svg` |
...
```bash
mkdir -p {hgs_re3設置パス}/public/img/review
...
class GenerateReviewOgpImage implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // 環境変数は明示的に渡す（PHP-FPMのclear_env対策）
        $result = Process::run(
        ...
        if ($result->successful()) {
            ...
            if ($json['ok'] ?? false) {
                // filenameのみ受け取り、パスはLaravel側で組み立てる
                $review->update([
...
'ogp' => [
    ...
    'output_dir'    => env('OUTPUT_DIR',          '{hgs_re3設置パス}/public/img/review'),
    'font_path'     => env('FONT_PATH',           '/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc'),
    'template_path' => env('SVG_TEMPLATE_PATH',   '{hgs_re3設置パス}/resources/ogp/review-template.svg'),
],
```
...
> PHP-FPMは `clear_env = yes` がデフォルトのため、Laravelの `.env` に設定しても環境変数がRustプロセスに自動継承されない。  
> そのため `Process::run()` の `env:` に明示的に渡している（上記のジョブコード参照）。  
> PHP-FPM側の設定変更は不要。

> **本番・STG共存について**  
> 同一サーバーに本番とSTGが同居している場合も、バイナリは共通（`/usr/local/bin/ogp-generator`）のまま運用できる。  
> 各LaravelのSTG用 `.env` で `OUTPUT_DIR` を本番と異なるパスに設定するだけで分離される。  
> `SVG_TEMPLATE_PATH` / `FONT_PATH` は共通のファイルを指しても問題ない。
...
- `review-template.svg` は `SVG_TEMPLATE_PATH`（Laravelのディレクトリ内）に配置する。このファイルは**Laravelのデプロイで管理**する。Rustツールのデプロイ（GitHub Actions）は関与しない
